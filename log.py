...
def sign_in():
    ...
    if username == 'khaled' and password1 == '1234':
        screen = Toplevel(root)
        screen.title('APP')
        screen.geometry('925x500+300+200')
        screen.config(bg='white')
        Label(screen, text='Hello Every one !', bg='#fff', font=(
            'Calibri(Body)', 50, 'bold')).pack(expand=True)

        screen.mainloop()
    elif username != 'khaled' and password1 != '1234':
        messagebox.showerror('Invalid', 'invalid username and password')

    elif password1 != '1234' and username == 'khaled':
...
Button(frame, width=39, pady=7, text='Sign in', command=sign_in,
